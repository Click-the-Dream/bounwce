...
const WithdrawAmountStep = ({ amount, setAmount, error }: any) => {
  const { useGetMyStore } = useStore();
  const { data, isLoading } = useGetMyStore();
  const [securityAnswer, setSecurityAnswer] = useState(
    isLoading ? "" : data?.payout_info?.security_answer,
  );
  ...
  return (
    <div>
      <div
        ...
        style={{ backgroundImage: `url(${withdrawBackgroudImg.src})` }}
      >
        <h1 className="text-white underline text-[12px]">
          Withdrawal Destination
        </h1>
        <p className="text-[10px]">
          <span className="text-white/65">Account Name: </span>
          <span className="text-white ">
            {isLoading ? "--" : data?.payout_info?.account_name}
          </span>
        </p>
        <p className="text-[10px]">
          <span className="text-white/65">Bank Name: </span>
          <span className="text-white ">
            {isLoading ? "--" : data?.payout_info?.bank_name}
          </span>
        </p>
        <p className="text-[10px]">
          <span className="text-white/65">Account Number: </span>
          <span className="text-white ">
            {isLoading ? "--" : data?.payout_info?.account_number}
          </span>
        </p>
      </div>
      ...
      <form className="mt-3">
        ...
        <div className="relative">
          <label className="font-semibold text-[10px]">
            ...
            {isLoading ? "--" : data?.payout_info?.security_question}
          </label>
          <input
          ...
          <div className="flex justify-end mt-2">
            <button
              className="bg-[#E5F0FF] text-[#2563EB] py-0.75 px-2.5 rounded-[50px] text-[8px]"
              type="button"
            >
