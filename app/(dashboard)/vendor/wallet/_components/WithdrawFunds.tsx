...
const WithdrawFunds = ({ onClose, isOpen, onWithdraw }: any) => {
  ...
  const handleProceed = () => {
    ...
    const minAmount = parseFloat("2000.00");
    const availableBalance = parseFloat("300.0");
    if (step === 1) {
      if (isNaN(numAmount)) {
    ...
    switch (step) {
      ...
      case 3:
        return (
          <WithdrawSucessStep
            amount={amount}
            bankName={data.payout_info.account_name}
            accountNumber={data.payout_info.account_number}
  ...
  return (
    ...
    >
      {/* modal section */}
      <div
        className={`bg-white rounded-lg p-5 transform transition-all duration-500 ease-in-out ${isOpen ? "scale-100 opacity-100" : "scale-50 opacity-0"}`}
      ...
      >
        ...
        {step === 3 ? (
          ...
          <p className="text-[11px] text-black/50 mb-3">
            Enter Withdrawal details and security information
          </p>
        )}
        ...
        <div className="flex gap-3 justify-end text-[10px] mt-8">
          <button
            className={`border p-2.25 rounded-[4.45px] ${step === 3 ? "hidden" : "block"}`}
            onClick={() => {
              if (step === 2) {
                setStep(1);
              } else {
          ...
          <button
            className="flex gap-1 items-center p-2.25 rounded-[4.45px] bg-black text-white"
