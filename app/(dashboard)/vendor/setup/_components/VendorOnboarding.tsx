"use client";
import { getFirstMissingStep } from "@/app/_utils/formatters";
import { OnboardingFormValues } from "@/app/_utils/types/vendor";
...
const VendorOnboarding = () => {
  ...
  const { data: onboardingStatus, isLoading: statusLoading } =
    useGetStoreOnboardingStatus(authDetails?.user?.id);
  const missingSections = onboardingStatus?.missing_sections || [];

  const { step: initialStep, tab: initialTab } =
    getFirstMissingStep(missingSections);
  ...
  const getTabFields = (tab: string): FieldPath<OnboardingFormValues>[] => {
    switch (tab) {
      case "store":
    ...
    try {
      if (currentTab === "store") {
        ...
        await action.mutateAsync(
          ...
          {
            onSuccess: () => {
              setCompletedTabs((prev: any) => [...prev, currentTab]);
