...
import useShipping from "@/app/hooks/use-shipping";
...
const StepThree = () => {
  ...
  const handleAddShipping = async () => {
    ...
    mutation.mutate(
      editIndex != null ? { id: editIndex, ...newShipping } : newShipping,
      {
        onSuccess: () => {
      ...
      {isLoading ? (
        ...
        <ul className="gap-4">
          {shippings.map((s, index) => (
            <li
              key={index}
              className="relative border border-[#DFDFDF] rounded-lg p-4 flex items-start gap-3 bg-[#DFDFDF]/60 mb-2"
            >
      ...
      <div className="border border-gray-200 rounded-lg p-4 space-y-6">
        ...
        <div>
          ...
          <Controller
            ...
            render={({ field }) => (
              <LocationInput
                {...field} // value & onChange are automatically connected
