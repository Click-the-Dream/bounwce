...
const StepTwo = ({ register, errors, trigger }: any) => {
  ...
  const sortCategory = categories?.map(
  ...
  const handleAddProduct = async () => {
    ...
    try {
      ...
      formData.append("amount", parseInt(getValues("amount")) as any);
      ...
      if (hasId) {
        ...
        await createProduct.mutate(formData as any, {
          onSuccess: () => reset(),
        });
      }
  ...
  const handleEditProduct = (index: string | number) => {
  ...
  const handleDeleteProduct = async (index: string | number) => {
          ...
          {products?.length > 0 && (
            <ul className="gap-4">
              {products.map((prod: any, index: number) => (
                <li
                  key={index}
                  className="relative border border-[#DFDFDF] rounded-lg p-4 flex items-start gap-3 bg-[#D9D9D969] mb-2"
                >
                  <div className="absolute top-2 right-3 flex gap-1">
                    <button
                      type="button"
                      onClick={() => handleEditProduct(index)}
                      className="cursor-pointer p-1 hover:bg-gray-200 rounded-[5px] border border-[#9E9E9E]"
                      title="Edit"
                    >
                      <FaRegEdit size={10} className="" />
                    </button>
                    <button
                      type="button"
