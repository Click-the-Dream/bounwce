...
const GettingStarted = ({ storeData, onNext, onBack, initialTab }: any) => {
  const isValidTab = brandingSteps.some((s) => s.key === initialTab);
  const [currentSubStep, setCurrentSubStep] = useState(
    isValidTab ? initialTab : "branding",
  );
  const {
    register,
    trigger,
  ...
  const handleNextSub = async () => {
    ...
    if (currentSubStep === "branding") {
      try {
        ...
        await updateBranding.mutateAsync(formData as any, {
          onSuccess: () => {
  ...
  const canSkipStep = (stepKey: string) => {
    ...
    <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
          ...
          return (
            <React.Fragment key={step.key}>
              ...
              {index < brandingSteps.length - 1 && (
                <div
                  className={`flex-1 h-0.5 mx-2 my-auto ${
      ...
      <div className="flex flex-wrap justify-between gap-2 items-center mt-8 text-sm">
        <button
          ...
          className="cursor-pointer flex gap-2 items-center px-4 py-2 text-gray-600 border rounded-lg hover:bg-gray-50"
        >
          <FaArrowLeft /> Back
        </button>
        ...
        {currentSubStep === "publish" ? (
          <div className="flex gap-3 ml-auto">
            <button
              ...
              className="cursor-pointer flex gap-2 items-center px-4 py-2 border border-gray-300 bg-black text-white rounded-lg hover:bg-black/90"
            >
            ...
            <button
              ...
              onClick={() => {
                ...
                activateStore.mutate(null as any, {
                  onSuccess: () => onNext(),
                });
              }}
              className="cursor-pointer flex gap-2 items-center px-4 py-2 bg-orange text-white rounded-lg hover:bg-orange/90 disabled:opacity-50 disabled:cursor-wait"
            >
              <IoRocketOutline />{" "}
          ...
          <div className="flex gap-3 ml-auto">
            {canSkipStep(currentSubStep) && (
              <button
                type="button"
                onClick={handleSkip}
                className="cursor-pointer flex gap-2 items-center px-4 py-2 border bg-black rounded-lg text-white hover:bg-black/90"
              >
                Skip <FaArrowRight />
              </button>
            )}
            ...
            <button
              ...
              className="cursor-pointer flex gap-2 items-center px-4 py-2 bg-orange text-white rounded-lg hover:bg-orange/90 disabled:opacity-60 disabled:cursor-not-allowed"
            >
