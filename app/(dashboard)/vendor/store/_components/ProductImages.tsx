...
const ProductImages = () => {
  ...
  useEffect(() => {
    ...
    const validFiles = images.filter((file: any) => file instanceof File);
    const urls = validFiles.map((file: Blob | MediaSource) =>
  ...
  const handleFilesChange = (e: {
    ...
    const newImages = [
      ...images.filter((f: any) => f instanceof File),
      ...validFiles,
    ].slice(0, 4);
  ...
  return (
    <div className="bg-white rounded-[10px] p-5">
      ...
      <p className="text-[13px] text-[#717182] mb-4">
        Add photos of your product (up to 4 images)
      </p>
          ...
          <div key={index} className="relative">
            {previews[index] ? (
              ...
              <label
                ...
                className="flex flex-col items-center justify-center w-full h-25 border-2 border-dashed rounded-xl cursor-pointer text-[#DFDFDF]"
              >
                <CiImageOn size={25} />
            ...
            <input
              ...
              accept="image/*"
              className="hidden"
              onChange={handleFilesChange as any}
            />
          </div>
        ))}
      </div>
