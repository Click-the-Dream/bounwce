...
const WaitlistInsight = ({ isLoading, joinedCount, animatedCount, progressPercent, waitlistData, hideAnalytics }) => {
    return (
        <motion.div
            ...
            viewport={{ amount: 0.4 }}
            className={`pt-10 ${!hideAnalytics ? 'absolute bottom-10 left-0 right-0 bg-black/40 text-white' : 'relative bg-white text-black'}  h-max w-full max-w-sm mx-auto text-center px-6`}
        >
            {isLoading ? (
