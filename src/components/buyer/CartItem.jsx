...
const CartItem = ({ vIdx, cart, openItem, removeItem, updateQuantity, toggleItem, saveForLater }) => {
    // console.log(cart);

    return (
        <div key={cart?.name} className="space-y-4">
            {cart?.items?.filter((i) => i.status !== "saved").length >
                0 && (
                ...
                .map((item, iIdx) => (
                    ...
                    >
                        ...
                        <motion.div
                            initial={false}
                            animate={{
                                height: openItem === vIdx ? "auto" : 0,
                                opacity: openItem === vIdx ? 1 : 0,
                            }}
